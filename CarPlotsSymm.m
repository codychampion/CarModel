...
while min(carChar(:,1))<gridpts && counter<gridpts*2
    ...
    for ii=1:numcars
        ...
        elseif carChar(ii,4)==2
            ...
            gapsf(gapsf>=0)=-inf;
            gapf=-1*(max(gapsf))-1;    %number of empty sites ahead
            gapveh=find(carChar(:,2)==carChar(ii,1)+gapf+1);
            
            if carChar(gapveh(:),6)==2
                gapf=max([gapf-1,0]);
            end
            
            if carChar(ii,3)>gapf && sum(grid(1,min([gridpts,max([carChar(ii,1)-vmax,1])]):carChar(ii,1)))==0 && sum(grid(1,max([carChar(ii,1),1]):min([gridpts,carChar(ii,1)+min([vmax,carChar(ii,3)+1])])))==0 && passfactor>carChar(ii,8)
                carChar(ii,3)=min([vmax,carChar(ii,3)+1]);
                carChar(ii,7)=-1;                
            else
                gapsf=carChar(ii,1)-find(grid(2,:)~=0);
